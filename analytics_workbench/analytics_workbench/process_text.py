...
class ProcessText(object):
    ...
    def gen_document_term_matrix(self,text_documents,vectorizer_type='Count',synonyms={},stem_type='lemmatize',
        ...
        if synonyms:
            ...
            text_series = text_series.apply(lambda text : multiple_replace(synonyms,text_documents,word_limit=True,flags=re.IGNORECASE))
    ...
    def gen_dtm_from_files(self,text_documents,vectorizer_type='Count',synonym_loc=None,stem_type='lemmatize',
        ...
        '''
        if synonym_loc:
            synonyms = self.load_synonyms(synonym_loc)
        else:
            synonyms = None
        if stop_words_loc:
            ...
            stop_words = None
        if vocabulary_loc:
            vocabulary = self.load_words_from_file(vocabulary_loc)
        else:
            vocabulary = None
        return self.gen_document_term_matrix(
            text_documents=text_documents,vectorizer_type=vectorizer_type,synonyms=synonyms,stem_type=stem_type,
            phrase_generation=phrase_generation,stop_words=stop_words,lower=lower,n_gram_range=n_gram_range,
            max_df=max_df,min_df=min_df,vocabulary=vocabulary,**kwargs
        )
