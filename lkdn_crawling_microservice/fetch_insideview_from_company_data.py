...
class API_counter(object):
    ...
    def get_list_api_counts(self):
        ...
        query = " select company_search_hits,company_details_hits,newcontact_search_hits,newcontact_email_hits" \
                ",people_search_hits,contact_fetch_hits " \
        ...
        if not tmp:
            ...
            self.people_search_hits,self.contact_fetch_hits = tmp[0][4],tmp[0][5]
    ...
    def create_api_counts(self):
        ...
        self.con.cursor.execute(query,(self.list_id,0,0,0,0,0,0,))
        self.con.commit()
...
class InsideviewCompanyFetcher(object):
    ...
    def fetch_data_crawler_process(self,list_id,filters_loc,max_comps_to_try=0,get_contacts=1,
        ...
        if search_contacts:
            ...
            comp_ids_not_searched = self.data_util.get_companies_for_contact_search(list_id,comp_contries_loc,1,
                                                         comp_ids_to_find_contacts_file_loc)
            if comp_ids_not_searched:
                contacts_list = self.search_contacts_from_company_ids(company_ids=comp_ids_not_searched,max_res_per_company=max_res_per_company,
                                                                      **filters_dic)
            ...
            contact_ids = self.data_util.get_contact_ids_for_email_find(list_id=list_id,comp_ids=comp_ids,
        ...
        if get_contacts:
            ...
            if new_contact_ids_file_loc:#if new contactids provided, directly fetch emails (might be costlier)
                new_contact_ids = self.data_util.fetch_new_contact_ids_to_fetch(list_id,comp_ids,
                                                       max_res_per_company=max_res_per_company,desig_list=desig_list,
                                                       new_contact_ids_file_loc=new_contact_ids_file_loc)
                self.fetch_people_details_from_newcontact_ids(new_contact_ids,retrieve_comp_dets= not get_comp_dets_sep)
            else:
    ...
    def upload_company_url_list_df_inp(self,url_df,list_id):
        ...
        :return:
        '''#company_name_field,website_field,country_field,state_field,city_field = 'company_name','website','country','state','city'
        company_dets = [tuple([list_id])+tuple(row[[company_name_field,website_field,country_field,state_field,city_field]])
...
if __name__ == "__main__":
    ...
    new_contact_ids_file = options.new_contact_ids_file
    # checks
    if not list_name:
    ...
    fetcher.fetch_data_csv_input(inp_loc=inp_loc,out_loc=out_loc,filters_loc=filter_loc,
                                 ...
                                 new_contact_ids_file_loc=new_contact_ids_file)
