...
toolconfig = cfg.readConfig(section='throttler')
...
headers = {'Accept':'application/json',
           ...
           #'accessToken':toolconfig['access_token']
           }
...
class RateLimitter(Resource):
    ...
    def get(self):
        ...
        # print('input dict:{}'.format(req_dic))
        ...
        try:
            logger.info('output dict:{}'.format(json.loads(r.text)))
            # print('output dict:{}'.format(json.loads(r.text)))
            return json.loads(r.text)
        except Exception as e:
            logger.error('Error : {}'.format(e))
            # print('error happened: {}'.format(e))
            return {'message':'No json object could be loaded from Insideview response'}
        # return {'value':1}
