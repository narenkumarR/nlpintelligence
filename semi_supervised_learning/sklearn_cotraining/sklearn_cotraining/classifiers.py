...
class CoTrainingClassifier(object):
    ...
    def fit(self, X1, X2, y_raw):
        ...
        U = U[:-min(len(U), self.u_)]


        n_iter = 0 #number of cotraining iterations we've done so far
        # import pdb
        # pdb.set_trace()
        #loop until we have assigned labels to everything in U or we hit our iteration break condition
        while n_iter != self.k_ and (U or U_):
            n_iter += 1
            ...
            y1_prob_0_ranks = rankdata(-y1_prob[:,0],method='min')
            y1_prob_1_ranks = rankdata(-y1_prob[:,1],method='min')
            y2_prob_0_ranks = rankdata(-y2_prob[:,0],method='min')
            y2_prob_1_ranks = rankdata(-y2_prob[:,1],method='min')
            ...
            #         break
                ...
                # paper does not say they have to match.
            ...
            # take the most confident positive and negative examples from each predicted probs and add them to n and p
            for i, ( y1_prob_i, y2_prob_i, y1_prob_0_rank_i, y1_prob_1_rank_i,y2_prob_0_rank_i,y2_prob_1_rank_i) in \
                    enumerate(zip( y1_prob[:,0], y2_prob[:,0],y1_prob_0_ranks,y1_prob_1_ranks,y2_prob_0_ranks,y2_prob_1_ranks)):
                #we added all that we needed to for this iteration, so break
                if len(p) == 2 * self.p_ and len(n) == 2 * self.n_:
                    break

                #update our newly 'labeled' samples.  Note that we are only 'labeling' a single sample
                #with each inner iteration.  We want to add 2p + 2n samples per outer iteration
                #
                if y1_prob_i < 0.5 and len(p) < 2 * self.p_ and y1_prob_1_rank_i <= self.p_:
                    p.append(i)
                elif y2_prob_i < 0.5 and len(p) < 2 * self.p_ and y2_prob_1_rank_i <= self.p_:
                    p.append(i)
                elif y1_prob_i >= 0.5 and len(n) < 2 * self.n_ and y1_prob_0_rank_i <= self.n_:
                    n.append(i)
                elif y2_prob_i >= 0.5 and len(n) < 2 * self.n_ and y2_prob_0_rank_i <= self.n_:
                    n.append(i)
                else:
                    continue
            ...
            # for i in p:U_.pop(i)
            # for i in n: U_.pop(i)
            del_list = list(set(p+n))
            del_list.sort(reverse=True)
            for i in del_list: U_.pop(i)
