...
class InsideviewContactFetcher(object):
    ...
    def search_contact_for_email(self,list_id,desig_loc=None,contact_ids_file_loc=None,n_threads=10):
        ...
        if contact_ids:
            ...
            self.fetch_people_details_from_contact_ids_job(contact_ids,n_threads)
        self.con.commit()
        ...
        def worker():
            while not in_queue.empty():
                try:
                    ...
                    break
                try:
                    logging.info('fetch_people_details_from_contact_ids: tying for contact_id:{}'.format(contact_id))
    ...
    def fetch_people_details_from_contact_ids_job(self,contact_ids,n_threads=10):
        '''
        :param new_contact_ids:
        :param retrieve_comp_dets:
        :param n_threads:
        ...
        def worker():
            while not in_queue.empty():
                try:
                    ...
                    break
                try:
                    logging.info('search_for_matching_people_from_ppl_details: trying for dets:{}'.format((dets_tuple,person_id)))
        ...
        def worker():
            while not in_queue.empty():
                try:
                    ...
                    break
                try:
                    logging.info('search_contact_for_people_threaded: trying for dets: {}'.format((list_input_id,search_dic)))
